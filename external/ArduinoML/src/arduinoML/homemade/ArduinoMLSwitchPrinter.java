	...
	public String caseApp(App object) {
		...
		if (monitoring) {
			...
			if (object.getName() != null) {
				...
				sb.append("\tSerial.println(\"modes=" + mode.getName() + "\");\n");
				sb.append("\tSerial.println(\"states=");
				ArrayList<String> s_names = new ArrayList<String>();
				for (int i = 0; i < mode.getStates().size(); i++) {
					if (i > 0) {
		...
		if (monitoring) {
			...
			if (inUniqueModeState) {
				sb.append("void out_" + object.getModes().get(0).getName() + "() {\n"
						+ "\tboolean time_analog = millis() - analog > debounce;\n"
						+ "\tif (time_analog) {\n"
						+ "\t\tSerial.println(");
				if (analogs_modes_names.get(0).size() > 0) {
					boolean first = true;
					sb.append("\"analog");
